...
function curl_to_py()
    request = vim.fn.input("Please enter full curl request (command included): ")
    curl = '"' ..request.. '"'
    vim.cmd(':enew | .!curlconverter -l python ' .. curl .. '')
end

vim.api.nvim_set_keymap('n', '<leader>c2p', '<cmd>lua curl_to_py()<CR>', options)

-- NVIM Rest client
vim.api.nvim_set_keymap('n', '<leader>rest', '<Plug>RestNvim<CR>', options)
